...
class evm
{
    ...
    public static function start_machine($machine)
    {
        ...
        if (self::is_running($machine)) {
            ...
            while ($acik < 1) {
                ...
                if ($komut == 0) {
                    shell_exec('optirun vboxmanage startvm "' . $machine . '" --type headless 2>&1 &');
                    $komut = 1;
                }
                //~ sleep(1);
            }
    ...
    public static function fused($clear = null)
    {
        /*
         * Aynı host ismini içeren başka makina varsa hata verecektir!!!
         * */
        $return = self::fused_machines($clear);
        ...
        if (!is_null($clear) && count($return) > 0) {
            ...
            echo "\n\e[42m\e[44m\e[1mAktif sshfs mount edilmiş sanal makine bulunamadı.\e[0m\n";
        }
    }

    public static function fused_machines($clear = null)
    {
        $return = array();
        foreach (self::running_machines() as $running) {
            exec("mount -l -t fuse.sshfs | grep '{$running}' | awk '{print $3}'", $fused);
            if (count($fused) > 0) {
                if (!is_null($clear)) {
                    $return[] = $running;
                } else {
                    $return[] = "\e[42m\e[44m\e[1mFused :\e[0m " . $running;
                }
            }
            $fused = array();
        }
    ...
    public static function fusermount_machine($machine)
    {
        if (self::is_fused($machine)) {
            exec("mount -l -t fuse.sshfs | grep '{$machine}' | awk '{print $3}'", $mount_point);
            $mount_point = implode(null, $mount_point);
            echo $machine . "\t" . $mount_point . "\n";
            exec("fusermount -u {$mount_point}");
        }
    }

    public static function is_fused($machine)
    {
        return self::is_machine($machine) && in_array($machine, self::fused_machines(true)) ?
            true :
            false;
    }

    public static function fusermount_all()
    {
        foreach (self::fused_machines(1) as $machine) {
            self::fusermount_machine($machine);
        }
        self::fused();
    }
...
/*
 * Get Fused File Systems
 * */
if ($islem == "get_fused") {
    evm::fused($clear ? 1 : null);
}

/*
 * umount Fused File Systems
 * */
if ($islem == "fusermount_machine") {
    foreach ($machines as $machine) {
        evm::fusermount_machine($machine);
    }
    evm::fused($clear ? 1 : null);
}

if ($islem == "fusermount_all") {
    evm::fusermount_all($clear ? 1 : null);
}
...
/*
 * Attach GUI To Machine(s)
 * Host + Space Detaches the GUI
 * */
